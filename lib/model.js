...
class Model {
    ...
    static create(modelNameOrId, _id) {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    static find(idOrFieldName, opStr, value) {
        if (idOrFieldName && opStr) {
            return this.findWhere(idOrFieldName, opStr, value);
        }
        else if (typeof idOrFieldName === 'string' && !opStr) {
            return this.findById(idOrFieldName);
        }
        else {
            return this.findAll();
        }
    ...
    static _normalizeElementSchema(key, schema) {
        if (schema) {
            if (interfaces_1.isModelFieldSchema(schema)) {
                if (!schema.key) {
                    schema.key = key;
                }
                return schema;
